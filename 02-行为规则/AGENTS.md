...
## 记忆系统：Hindsight 持久化后端

本系统使用 Hindsight 作为主要记忆后端，MEMORY.md 作为兜底备份。

### 记忆实时同步规则
- 任何需要跨会话保留的信息，在写入 MEMORY.md 后，Hindsight 自动同步
- Hindsight 支持**会话内实时检索**，写入后立即可查
- 三层记忆架构：
  - **热记忆**：MEMORY.md（8000 char，<30天高频使用）
  - **温记忆**：`~/.hermes/memory/warm_summary.md`（30-90天摘要）
  - **冷记忆**：`~/.hermes/memory/cold_archive/`（90天以上归档）
- 当 MEMORY.md 超过 7000 字符时，主动将低频内容移至温/冷记忆

### 记忆写入纪律
- 万手哥说"记住"、"记下来"等内容 → 优先保证写入 MEMORY.md
- 发现记忆不准时 → 告知万手哥触发索引重建
- 不主动删除记忆，除非万手哥明确要求
